...
access(all) contract IncrementFiStakingConnectors {
    access(all) var __mockIdCounter: UInt64
    access(all) var __mockPoolSinks: {UInt64: MockPoolSink}
    access(all) var __mockPoolRewardsSources: {UInt64: MockPoolRewardsSource}

    init() {
        self.__mockIdCounter = 0
        self.__mockPoolSinks = {}
        self.__mockPoolRewardsSources = {}
    }

    access(all) struct MockPoolSink {
        access(all) let __values: {String: AnyStruct}

        init(_ values: {String: AnyStruct}) {
            self.__values = values
        }

        access(all) fun getVaultType(): Type {
            return self.__values["vaultType"] as! Type
        }

        access(all) fun getUniqueID(): DeFiActions.UniqueIdentifier? {
            return self.__values["uniqueID"] as! DeFiActions.UniqueIdentifier?
        }
    }

    access(all) struct Mock {
        access(all) let values: {String: AnyStruct}
        access(all) let mockPoolSink: MockPoolSink
        access(all) let mockPoolRewardsSource: MockPoolRewardsSource

        init() {
            self.mockPoolSink = MockPoolSink()
            self.mockPoolRewardsSource = MockPoolRewardsSource()
        }

        access(contract) fun createMockPoolSink(pid: UInt64, staker: Address, uniqueID: DeFiActions.UniqueIdentifier?): UInt64 {
            let mockId = IncrementFiStakingConnectors.__mockIdCounter
            IncrementFiStakingConnectors.__mockIdCounter = mockId + 1
            IncrementFiStakingConnectors.__mockPoolSinks[mockId] = MockPoolSink(
                pid: pid,
                staker: staker,
                uniqueID: uniqueID
            )
        ...
        access(contract) fun getMockPoolSink(mockId: UInt64): &MockPoolSink {
            return &IncrementFiStakingConnectors.__mockPoolSinks[mockId]!
        }
    }

    access(all) let mock: Mock 
    ...
    access(all) struct PoolSink: DeFiActions.Sink {
        access(self) let __mockId: UInt64
        access(self) let __mock: Mock
        ...
        init(
            ...
            // Create mock
            self.__mockId = IncrementFiStakingConnectors.mock.createMockPoolSink(
                pid: pid,
                staker: staker,
                uniqueID: uniqueID
            )

            let mock = IncrementFiStakingConnectors.mock.getMockPoolSink(mockId)

            self.vaultType = mock.vaultType
            self.uniqueID = mock.uniqueID
        }
